...
export const productApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        addProductitemToCart: builder.mutation({
            query: ({ productitemId, qty, productConfig }) => ({
                url: '/api/cart',
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: { productitemId, qty, productConfig },
            }) 
        }),
        ...
        removeCartItem: builder.mutation({
            query: ({ cartitemId, qty, productitemId }) => ({
                url: `/api/cart/${cartitemId}?qty=${qty}&productitemId=${productitemId}`,
                method: 'DELETE',
            })
        })
    })
...
export const { useGetCategoriesQuery, 
    ...
    useGetProductitemsWithProductIdQuery, 
    useGetProductitemWithIdQuery, 
    useGetProductConfigWithIdQuery, 
    useGetProductReviewsWithIdQuery,
    useAddProductitemToCartMutation,
    useGetCartItemsQuery, 
    useRemoveCartItemMutation } = productApi